...
void ConfigParser::build_map(void)
{
	_nb_servers = servers.size();
	for (size_t i = 0; i < servers.size(); i++)
	{
		std::map<std::string, std::string> parameters = serverParameters[i];

		 _server_map[i]["listen"] =  getListenValue(serverParameters[i]);
		_server_map[i]["server_name"] = getServerName(serverParameters[i]);
		_server_map[i]["body_size"] = getBodySize(serverParameters[i]);
		_server_map[i]["error_page_number"] = getErrorNumber(serverParameters[i]);
		_server_map[i]["error_location"] = getErrorLocation(serverParameters[i]);
		_server_map[i]["location"] = getLocation(serverParameters[i]);
		
		if (parameters["GET"].compare("Get") == 0)
			_server_map[i]["allow_GET"] = "y";
		else
			_server_map[i]["allow_GET"] = "n";

		if (parameters["POST"].compare("Post") == 0)
			_server_map[i]["allow_POST"] = "y";
		else
			_server_map[i]["allow_POST"] = "n";

		if (parameters["DELETE"].compare("Delete") == 0)
			_server_map[i]["allow_DELETE"] = "y";
		else
			_server_map[i]["allow_DELETE"] = "n";

		_server_map[i]["noAllowedMethods"] = parameters["NoAllowedMethods"];

		_server_map[i]["autoindex"] = parameters["autoindex"];
		_server_map[i]["index"] = parameters["indexing"];
		_server_map[i]["cgi"] = parameters["cgi"] ;
	}
}
